...
- 通过 `筛选自动拉取的模型` 可仅包含选中的模型 ID，同时继续从供应商刷新这些模型的参数。
...
| 自动拉取模型筛选         | `autoFetchOfficialModelsFilter`                  | 自动拉取模型 ID 的可选白名单。省略时包含所有官方模型；空数组表示不包含任何官方模型。                                                                                                                  |
...
| **Google**       | Gemini 3.7 系列     | gemini-3.7-flash |
