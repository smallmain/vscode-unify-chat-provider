...
import { performXaiGrokAuthorization } from './screens/authorize-screen';
...
export class XaiGrokOAuthProvider implements AuthProvider {
  static supportsSensitiveDataInSettings(_auth: XaiGrokOAuthConfig): boolean {
    return false;
  }

  static redactForExport(auth: XaiGrokOAuthConfig): XaiGrokOAuthConfig {
  ...
  static async resolveForExport(
    auth: XaiGrokOAuthConfig,
    secretStore: SecretStore,
  ): Promise<XaiGrokOAuthConfig> {
  ...
  static async prepareForDuplicate(
    ...
    const cleared: XaiGrokOAuthConfig = {
  ...
  constructor(
    ...
    private config?: XaiGrokOAuthConfig,
  ) {}

  get definition(): AuthProviderDefinition {
    return {
      id: 'xai-grok-oauth',
      label: this.config?.label ?? 'xAI Grok OAuth (SuperGrok)',
      description:
        this.config?.description ??
        t('Authenticate using xAI Grok OAuth (SuperGrok / X Premium+)'),
    };
  }

  getConfig(): XaiGrokOAuthConfig | undefined {
    return this.config;
  }
  ...
  private async resolveTokenData(): Promise<OAuth2TokenData | null> {
    ...
    return XaiGrokOAuthProvider.parseTokenData(raw);
  }
  ...
  async getCredential(): Promise<AuthCredential | undefined> {
    authLog.verbose(`${this.context.providerId}:xai-grok-oauth`, 'Getting credential');

    const token = await this.resolveTokenData();
    if (!token) {
      authLog.verbose(`${this.context.providerId}:xai-grok-oauth`, 'No token data available');
      return undefined;
    }
  ...
  async configure(): Promise<AuthConfigureResult> {
    ...
    const authorization = authorizeXaiGrok();
    ...
    const callbackResult = await vscode.window.withProgress(
      ...
      async (_progress, token) => {
        return performXaiGrokAuthorization({
    ...
    const exchanged = await exchangeXaiGrokCode({
  ...
  async refresh(): Promise<boolean> {
    ...
    if (!token?.refreshToken) {
      authLog.error(`${this.context.providerId}:xai-grok-oauth`, 'No refresh token available');
      return false;
    }

    const refreshed = await refreshXaiGrokToken({ refreshToken: token.refreshToken });
    if (refreshed.type === 'failed') {
      authLog.error(`${this.context.providerId}:xai-grok-oauth`, `Refresh failed: ${refreshed.error}`);
    ...
    const raw = this.config?.token?.trim();
    if (!raw) {
      authLog.error(`${this.context.providerId}:xai-grok-oauth`, 'No token reference in config');
      return false;
    }
