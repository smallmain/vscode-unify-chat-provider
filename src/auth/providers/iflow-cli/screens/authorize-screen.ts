...
import { t } from '../../../../i18n';
import { IFLOW_CALLBACK_PORT, IFLOW_REDIRECT_PATH } from '../constants';
...
export async function performIFlowAuthorization(options: {
  url: string;
  ...
  const manualFallback = async (): Promise<CallbackResult | null> => {
    ...
    if (parsed.pathname !== IFLOW_REDIRECT_PATH) {
  ...
  try {
    const started = await mainInstance.runInLeaderWhenAvailable<{
      ...
      'oauth.http.start',
      {
        port: IFLOW_CALLBACK_PORT,
        redirectPath: IFLOW_REDIRECT_PATH,
