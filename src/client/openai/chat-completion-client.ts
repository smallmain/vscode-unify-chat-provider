...
export class OpenAIChatCompletionProvider implements ApiProvider {
  ...
  private convertUserMessage(
    ...
    for (const part of message.content) {
      ...
      const converted = this.convertPart(message.role, part);
      if (!converted) {
        continue;
      }
      if (Array.isArray(converted) || converted.role !== 'user') {
        throw new Error('Expected user content outside a tool result');
      }
