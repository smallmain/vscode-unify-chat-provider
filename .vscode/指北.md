...
- `acquire` 接受可选的 `AbortSignal`：若调用前已被 abort，或在等待期间被 abort，则拒绝并抛出 `name: 'AbortError'` 的错误，**不消耗令牌**。服务层把 VS Code 的 `CancellationToken` 转成 `AbortSignal` 传入，避免用户取消请求后仍占用一个令牌槽。
...
5. 设置较低的 `rpm`（例如 2），立即发送若干请求后随即取消：被取消的请求应不消费令牌、不会随后才完成；输出通道对其记为 `cancelled`。
