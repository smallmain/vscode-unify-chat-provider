...
- `RateLimiter.acquire(signal?)` 使用 FIFO Promise 队列串行化“补充、检查、等待、消费”，防止并发等待者消费同一个新补充的令牌或把令牌数扣为负数。
- `acquire` 接受可选的 `AbortSignal`：若调用前已被 abort，或在等待期间被 abort，则拒绝并抛出 `name: 'AbortError'` 的错误，**不消耗令牌**。即使该请求仍排在其他等待者后面，调用方也会立即结束；取消项轮到队首时仅退出队列。服务层把 VS Code 的 `CancellationToken` 转成 `AbortSignal` 传入，避免用户取消请求后仍占用一个令牌槽。
- `getAvailableTokens()` 不再在返回前补充令牌；`acquire()` 内部已先补充再消费，因此紧随其后的调用返回的是精确的消费后快照。
...
- 保留 `ApiProvider.acquireRateLimitToken(signal?)` 与 `getRateLimitStatus()`，由服务层统一调用；`acquireRateLimitToken` 必须可取消。
- `config-store` 通过 `normalizeRateLimitConfig()` 校验供应商级 `rateLimit`：非对象、`rpm` 缺失或非整数会被归一化为 `undefined`，从而回落到全局默认值；显式 `rpm: 0` 仍被视为“关闭限流”并被保留，不回落到全局。
...
5. 设置较低的 `rpm`（例如 2）并耗尽令牌，再发起一个等待请求；随后发起第二个请求并立刻取消：第二个请求应立即结束，不消费令牌，也不必等待前一个请求获得令牌；输出通道对其记为 `cancelled`。
6. 把供应商级 `rateLimit` 设为 `{}` 或 `{ "rpm": "abc" }`，同时在全局设置有效的 `rpm`：供应商应回落到全局默认值生效。
