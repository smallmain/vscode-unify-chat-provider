...
describe('DeepSeek vision Chat Completions message conversion', () => {
  ...
  it('preserves legacy tool results and skips internal metadata', () => {
    const converted = provider.convertForRequest([
      userMessage([
        new vscode.LanguageModelToolResultPart('legacy-call', [
          new vscode.LanguageModelTextPart('Legacy result.'),
        ]),
        toolResult('image-call', [
          new vscode.LanguageModelDataPart(new Uint8Array(), 'stateful_marker'),
          imagePart(1),
        ]),
      ]),
    ]);
    ...
    expect(converted).toEqual([
      { role: 'tool', tool_call_id: 'legacy-call', content: 'Legacy result.' },
      { role: 'tool', tool_call_id: 'image-call', content: '' },
      {
        role: 'user',
        content: [
  ...
  it('rejects unsupported tool result data instead of losing it silently', () => {
    expect(() => provider.convertForRequest([
      userMessage([
        toolResult('call-1', [
          new vscode.LanguageModelDataPart(new Uint8Array([1]), 'audio/wav'),
        ]),
      ]),
    ])).toThrow('Unsupported from_tool_result message LanguageModelDataPart mime type: audio/wav');
  });
});
